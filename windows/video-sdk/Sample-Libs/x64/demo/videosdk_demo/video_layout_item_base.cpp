...
void VideoLayoutItemBase::OnMouseHover(bool is_hover)
{
	if (lower_third_name_.empty())	
		return;
	
	if (is_mouse_hover_ == is_hover)
		return;
	else
		is_mouse_hover_ = is_hover;

	if (is_hover)
		UpdateMouseHoverLowerThird();
	else
		UpdateLowerThird();
}
...
Gdiplus::Bitmap* VideoLayoutItemBase::GetUserNameBitmap(int max_width, int muted_width)
{
	...
	if(user_->GetSharePipe())
		share_status = user_->GetSharePipe()->getShareStatus();
