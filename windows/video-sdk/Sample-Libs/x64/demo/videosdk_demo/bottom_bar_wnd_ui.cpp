...
void CBottomBarWndUI::DoFeedBackBtnClick()
{
	if (!label_feedback_num_ || !btn_feedback_)
		return;

	label_feedback_num_->SetBkImage(NUMBER_IMAGE_BLACK);

	RECT rcScreen = GetButtonPos(btn_feedback_);
	CMainFrame::GetInstance().ShowFeedBackLaunchWnd(true, rcScreen);
}
...
void CBottomBarWndUI::DoShareBtnClick()
{

	RECT rcScreen = GetButtonPos(btn_share_);

	CMainFrame::GetInstance().ShowShareSelectAppsWnd(true, rcScreen);
}

void CBottomBarWndUI::DoEmojiBtnClick()
{
	RECT rcScreen = GetButtonPos(btn_emoji_);
	CMainFrame::GetInstance().ShowEmojiBarWnd(true, rcScreen);
}

void CBottomBarWndUI::DoMoreBtnClick()
{
	RECT rcScreen = GetButtonPos(btn_more_);

	CMainFrame::GetInstance().ShowMorePopWnd(true,rcScreen);	
}
