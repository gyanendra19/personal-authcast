...
public:
	...
	virtual void onRemoteControlStatus(IZoomVideoSDKUser* pUser, ZoomVideoSDKRemoteControlStatus status) = 0;

	/// \brief Callback for when the current user has received a remote control request.
	/// This will be triggered when another user requests control of the current user's screen.
	/// \param pUser The pointer to the user who sent the request, see \link IZoomVideoSDKUser \endlink.
