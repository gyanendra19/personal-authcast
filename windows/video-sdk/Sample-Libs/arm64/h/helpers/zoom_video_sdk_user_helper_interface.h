...
public:
	...
	/// \remarks For custom style mode, this interface is only valid after subscribing to the content to be shared by the specified user by calling ICustomizedShareRender::SetUserID(unsigned int userid) successfully.
...
public:
...
#if defined(WIN32) || defined(__MACOS__)
	/// \brief Get the helper class instance to access the remote control. Only available for controller side. 
	/// \return If the function succeeds, the return value is the remote control helper object.
	///Otherwise returns NULL. For more details, see \link IZoomVideoSDKRemoteControlHelper \endlink.
	...
	/// \return If the function succeeds, the return value is the list of users I have agreed to control. Otherwise returns NULL.
	...
	/// \return If the function succeeds, the return value is the user object. Otherwise NULL.
