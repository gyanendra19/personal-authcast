...
LRESULT CControlCameraTipWndUI::OnMouseHover(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	RECT rcWindow = { 0 };
	::GetWindowRect(this->GetHWND(), &rcWindow);
	POINT pt_temp = { rcWindow.left, rcWindow.top};
	CMainFrame::GetInstance().ShowControlCameraPopWnd(true, pt_temp);
